...
const randomWord = require('../lib/wordOfTheDay').randomWord
const letsPlay = require('./letsPlay').letsPlay
...
runForData = (play) => {
    if (wordValidation(word)) {
        ...
        play ? http.data(word.toLowerCase(), keys, play) : http.data(word.toLowerCase(), keys)
    } else {
        console.log('')
...
module.exports = {
     run: () => {
        program
            ...
            console.log('---')
            console.log('------')
            console.log('Thanks for using Words CLI.');
            console.log('')
